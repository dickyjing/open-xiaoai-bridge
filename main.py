...
def setup_config():
    """解析命令行参数和环境变量"""
    global connect_xiaozhi, enable_api_server, enable_xiaozhi, enable_openclaw, enable_openai, enable_hermes
    ...
    logger.info(
        f"Hermes: {'启用' if enable_hermes else '禁用'}",
        module="Main",
    )
    logger.info(
...
def run_services(xiaozhi_mode: bool = False):
    ...
    """
    global main_app_instance, enable_api_server, enable_openclaw, enable_openai, enable_hermes
